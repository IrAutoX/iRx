...
void addchange(const char *desc, int type)
{
    ...
    if(type!=CHANGE_GFX)
    {
        conoutf("..restart IrAutoX for this setting to take effect");
        return;
    }
    bool changed = false;
...
void refreshapplymenu(void *menu, bool init)
{
    ...
    m->items.add(new mitemtext(m, newstring("No"), newstring("echo [..restart IrAutoX to apply the new settings]"), NULL, NULL));
