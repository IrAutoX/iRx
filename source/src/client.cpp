...
void current_version(char *text)
{
    ...
    {
        hudoutf("\f3YOUR VERSION OF IrAutoX IS OUTDATED!");
        conoutf("\f3YOU MUST UPDATE IrAutoX\nplease visit \f2http://irautox.ir/irx \f3for more information");
    }
}
...
void gets2c()           // get updates from the server
{
    ...
    {
        conoutf("Dar Hal Connect Dadan Beh Server...");
        connmillis = totalmillis;
        ++connattempts;
        if(connattempts > 3)
        {
            conoutf("\f3Beh Server Nmiseh Connect Dad!");
            abortconnect();
            return;
        }
    }
    ...
    switch(event.type)
    {
        case ENET_EVENT_TYPE_CONNECT:
            ...
            connpeer = NULL;
            connected = 1;
            conoutf("Connect Shodi Beh Server");
        ...
        case ENET_EVENT_TYPE_RECEIVE:
        {
            ...
            if(discmillis) conoutf("Dar Hal Talash Baray Disconnect...");
        ...
        case ENET_EVENT_TYPE_DISCONNECT:
        {
            ...
            {
                conoutf("\f3Nmiseh Beh Server Connect Dad!");
                abortconnect();
            }
            else
            {
                if(!discmillis || event.data) conoutf("\f3Nett Ghat Shod , disconnecting (%s) ...", disc_reason(event.data));
                disconnect();
            }
            return;
        }
...
bool securemapcheck(const char *map, bool msg)
{
    ...
    loopv(securemaps) if(!strcmp(securemaps[i], map))
    {
        if(msg)
        {
            conoutf("\f3Map \f4%s\f3 In Map Ghabel Edit Nmibashad.", map);
            if(curpeer)
            {
                conoutf("\f3Agar In Error Ra Daryaft Kardid Hatman Noskeh Shoma Ya Server Ghadimi Mimashad");
                conoutf("\f3Shoma Mitavanid Noskeh Jadid Ra Az \f1http://irautox.ir/irx Daryaft Konid");
            }
        }
        return true;
    }
...
void sendmap(char *mapname)
{
    ...
    {
        conoutf("map %s Por Hajm Ast", mapname);
        delete[] mapdata;
    ...
    conoutf("Ferstadan Map %s Beh Server...", mapname);
}
COMMAND(sendmap, "s");
...
void getmap(char *name, char *callback)
{
    ...
    {
        conoutf("Dar Hal Daryaft Map Az Server...");
    ...
    else
    {
        ...
        if(downloadpackages(false))
        {
            ...
            conoutf("map %s Ba Movafaghit Load Shod", name);
        }
        else conoutf("\f3map download Nashod");
    }
}
COMMAND(getmap, "ss");
