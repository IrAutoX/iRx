...
}void raydamage(vec &from, vec &to, playerent *d)
{
    ...
    if(d->weaponsel->type==GUN_SHOTGUN)
    {
        ...
        loopi(SGRAYS)
        {
            ...
            if(hits[h])
            {
                ...
                int numhits_o=0,numhits_m=0,numhits_c=0;
                ...
                if(SGDMGBONUS)
                {
                    ...
                    if(o)
                        d2o=vec(from).sub(o->o).magnitude();
        ...
        if(hitscount)
            shorten(from,to,hitdistsquared);
    }
    else if((o=intersectclosest(from,to,d,distsquared,hitzone)))
    {
        ...
        {
            if(dam>=100)
                hudoutf("\f1+%d DAMAGE",dam);
            else if(dam>=50)
                hudoutf("\f3+%d DAMAGE",dam);
            else
                hudoutf("\f0+%d DAMAGE",dam);
        }
    ...
    if(d==player1 && multiplayer(NULL))
    {
        ...
        if(hit)
            accuracym[d->weaponsel->type].hits+=(float)hitscount/rayscount;

        accuracym[d->weaponsel->type].shots++;
    }
}

const char *weapstr(int i)
{
    return valid_weapon(i) ? guns[i].title : "x";
}
