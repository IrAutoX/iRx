...
extern IrAutoX::Client irx;
extern bool irxReady;
void cleanup(char *msg)         // single program exit point;
{
    ...
    {
        #ifdef WIN32
        MessageBox(NULL, msg, "IrAutoX fatal error", MB_OK|MB_SYSTEMMODAL|MB_ICONERROR);
        #else
        printf("%s", msg);
        #endif
        if(clientlogfile)
...
void setupscreen(int &useddepthbits, int &usedfsaa)
{
    ...
    loopi(sizeof(configs)/sizeof(configs[0]))
    {
        ...
        screen = SDL_CreateWindow("IrAutoX",
...
void connectprotocol(char *protocolstring) // IrAutoX://example.org[:28763][/][?[port=28763][&password=secret]]
{
    urlparse u;
    u.set(protocolstring);
    if(strcmp(u.scheme, "IrAutoX") || !*u.domain) { conoutf("\f3bad commandline syntax (\"%s\")", protocolstring); return; }
...
int main(int argc, char **argv)
{
    ...
    for(int i = 1; i<argc; i++)
    {
        ...
        if(!scl.checkarg(argv[i]))
        {
            ...
            else if(!strncmp(argv[i], "IrAutoX://", 14)) // browser direct connection
            {
                waitforauth = true;
...
if(!dedicated)
{
    ...
    options.gameId = 2;
    options.gameName = "iRx Shooter";
    options.autoStartLauncher = true;

    irxReady = irx.initialize(options);
    ...
    if(irxReady)
    {
        ...
        presence.details = "Playing iRx Shooter";

        irx.setPresence(presence);
    }
}
initserver(dedicated);
...
#ifdef __APPLE__
    // Fix https://github.com/IrAutoX/AC/issues/300
    ...
    if(!notexture) fatal("could not find core textures (hint: run IrAutoX from the parent of the bin directory)");
...
if(initscript) execute(initscript);
	...
	for(;;)
{
    static int lastRPCUpdate = 0;

    if(totalmillis - lastRPCUpdate > 1000)
    {
        lastRPCUpdate = totalmillis;

        if(player1 && player1->name[0])
        {
            if(IsDiscordInGame())
            {
                const char* serverName = currentserver(5);

                static char serverInfo[128];
                ...
                {
                    const char* ip = currentserver(1);
                    const char* port = currentserver(3);
