...
{
    std::ifstream file(cfgfile, std::ios::binary);
    if(!file.is_open())
    {
    ...
    if(size <= 16) { file.close(); return; }
    std::vector<char> buffer(size);
    if(!file.read(buffer.data(), size)) { file.close(); return; }
    file.close();

    if(memcmp(buffer.data(), "IrAutoX", 7) == 0)
    {
        unsigned char key = buffer[7];
        char *data = buffer.data() + 16;
        size_t datalen = size - 16;
