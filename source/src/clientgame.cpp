...
{
    if(curpeer && curpeer->state == ENET_PEER_STATE_CONNECTED)
        return true;
...
inline const char * spawn_message()
{
    ...
    else if (multiplayer(NULL)) return "4Sabor Bash. \f2Feshar Nakhor Azizam!";
    else return ""; // theres no waiting for permission in sp
    // Despite its many glaring (and occasionally fatal) inaccuracies, IrAutoX itself has outsold the
    ...
    else if(player1->state==CS_DEAD || player1->state==CS_SPECTATE)
    {
        ...
        else
        {
            ...
            if(lastmillis>respawnmillis)
            {
                ...
                if(m_arena)
                {
                    if(!arenaintermission) hudeditf(HUDMSG_TIMER, "Ta Round Badi Sabor Bashid...");
...
void dokill(playerent *pl, playerent *act, bool gib, int gun)
{
    ...
    if(act == player1 && pl != act)
    {
        if(lastmillis - killstreaktime > 5000) killstreak = 0;

        killstreak++;
        killstreaktime = lastmillis;

        switch(killstreak)
        {
            case 2:
                hudoutf("\f3DOUBLE KILL!");
                audiomgr.playsound(S_DUB, SP_HIGH);
                break;

            case 3:
                hudoutf("\f3TRIPLE KILL!");
                audiomgr.playsound(S_TRIPLEKILL, SP_HIGH);
                break;

            case 4:
                hudoutf("\f3MULTI KILL!");
                audiomgr.playsound(S_MULTIKILL, SP_HIGH);
                break;

            case 5:
                hudoutf("\f3MONSTER KILL!");
                audiomgr.playsound(S_MONSTERKILL, SP_HIGH);
                break;

            case 6:
                hudoutf("\f3UNSTOPPABLE!");
                audiomgr.playsound(S_UNSTOPPABLE, SP_HIGH);
                break;

            case 7:
                hudoutf("\f3GODLIKE!");
                audiomgr.playsound(S_GODLIKE, SP_HIGH);
                break;

            case 8:
                hudoutf("\f3THE FATHER OF KILLERS!");
                audiomgr.playsound(S_GODLIKE, SP_HIGH);
                break;
        }
    }
...
void flagmsg(int flag, int message, int actor, int flagtime)
{
    ...
    switch(message)
    {
        ...
        case FM_KTFSCORE:
        {
            ...
            int m = flagtime / 60;
            if(m)
                hudoutf("\f2%s%s%s Parcham Ro Baray %d min %d s Negah Dasht", tc, ta, tb, m, flagtime % 60);
            else
                hudoutf("\f2%s%s%s Parcham Ro Baray %d s Negah Dasht", tc, ta, tb, flagtime);
            break;
        }
...
void displayvote(votedisplayinfo *v)
{
    ...
    DELETEP(curvote);
    curvote = v;
    conoutf("%s Yek Vote Ejad Kard: %s", v->owner ? colorname(v->owner) : "", curvote->desc);
...
void callvoteerr(int e)
{
    ...
    conoutf("\f3Vote Ejad Nmiseh!: %s", voteerrorstr(e));
    DELETEP(calledvote);
}
