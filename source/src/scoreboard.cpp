...
void reorderscorecolumns()
{
    ...
    if(m_flags_) sscore.addcol(sc_flags, "Parcham");
    sscore.addcol(sc_frags, "Kills");
    sscore.addcol(sc_deaths, "deaths");
    sscore.addcol(sc_ratio, "Sath");
    if(multiplayer(NULL) || watchingdemo) sscore.addcol(sc_lag, "ping");
    sscore.addcol(sc_clientnum, "cn");
    sscore.addcol(sc_name, "ESM");
...
void renderscores(void *menu, bool init)
{
    ...
    if(getclientmap()[0])
    {
        ...
        formatstring(modeline)("\"%s\" Dar Mape %s", modestr(gamemode, modeacronyms > 0), fldrprefix ? getclientmap()+strlen("maps/") : getclientmap());
    }
    ...
    if((gamemode>1 || (gamemode==0 && (multiplayer(NULL) || watchingdemo))) && minutesremaining >= 0)
    {
        if(!minutesremaining)
        {
            ...
            if (m_teammode) // Add in the winning team
            {
                switch(winner)
                {
                    case TEAM_CLA: concatstring(modeline, ", \f3CLA Bordand!"); break;
                    case TEAM_RVSF: concatstring(modeline, ", \f1RVSF Bordand!"); break;
                    case -1:
                    default:
            ...
            else // Add the winning player
            {
                ...
                else concatformatstring(modeline, ", \f1%s Bord!", scores[0]->name);
            }
        }
        else
        {
            ...
            else concatformatstring(modeline, " BaghiMande");
        }
    }
...
const char *asciiscores(bool destjpg)
{
    ...
    if(destjpg)
    {
        formatstring(text)("IrAutoX Screenshot (%s)\n", asctimestr());
        addstr(t, e, text);
    }
    if(getclientmap()[0])
    {
        formatstring(text)("\n\"%s\" Dar Map %s", modestr(gamemode, 0), getclientmap());
        addstr(t, e, text);
    }
    if(multiplayer(NULL))
    {
...
//         const char *sr = scoreratio(d.frags, d.deaths);
    ...
    if(destjpg)
    {
        ...
        formatstring(text)("(%skills/deaths), %d minute%s remaining\n", m_flags_ ? "flags/" : "", minutesremaining, minutesremaining == 1 ? "" : "s");
        addstr(t, e, text);
    }
    return buf + 8;
}
